...
struct destinfo_t {
    ...
    uint16_t *clients;              /// Indexes of clients served by this proxy
    int status;                     /// Specified by a client_status value
    int comp_status;                /// Completion status as given by COMPLETE
...
extern int weight, min_time, max_rate, rate, port, rcvbuf, packet_wait;
extern int noname, unicast, client_auth, quit_on_error, dscp, follow_links;
extern int save_fail, restart_groupid, sync_mode, sync_preview, dest_is_dir;
...
extern char basedir[MAXDIR][MAXDIRNAME];
